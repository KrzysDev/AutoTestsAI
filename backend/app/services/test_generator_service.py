...
class TestGeneratorService:
    ...
    async def generate_html_test_from_prompt(self, prompt: str):
        ...
        classification: str = await self.ai_service.ask(classification_prompt, "google/gemma-4-31b-it:free")
        ...
        if "request" in classification.lower():
            ...
            parsed_prompt, tokens_used = await self.__ask_model_for_json(planning_parsing_prompt, ParsedPrompt, model="google/gemma-4-31b-it:free")
            ...
            generated_test = await self.ai_service.ask(combined_prompt, "google/gemma-4-31b-it:free")
            ...
            check_result = await self.ai_service.ask(checking_prompt, "google/gemma-4-31b-it:free")
            ...
            if check_result.strip().upper() != "OK":
                ...
                generated_test = await self.ai_service.ask(fixing_prompt, "google/gemma-4-31b-it:free")
                total_tokens += self.__count_tokens(generated_test)
    ...
    async def __handle_general_response(self, prompt: str, start: float, total_tokens: int):
        ...
        res = await self.ai_service.ask(gen_prompt, "google/gemma-4-31b-it:free")
    ...
    async def generate_html_test_from_survey(self, form: Form) -> TestGeneratorHTMLResponse:
        ...
        generated_test_raw = await self.ai_service.ask(combined_prompt, "google/gemma-4-31b-it:free")
        ...
        if not check_result.strip().upper().startswith("OK"):
            ...
            generated_test_raw = await self.ai_service.ask(fixing_prompt, "google/gemma-4-31b-it:free")
            total_tokens += self.__count_tokens(generated_test_raw)
    ...
    async def __ask_model_for_json(self, prompt: str, schema, max_tries: int = 3, model: str = "google/gemma-4-31b-it:free") -> tuple:
