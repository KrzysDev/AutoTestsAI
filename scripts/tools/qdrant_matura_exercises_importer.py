...
def main():
    ...
    print(f"Rozpoczynam import {len(data)} zadań do Qdrant...")
    ...
    for i, item in enumerate(data):
        ...
        try:
            ...
            print(f"[{i+1}/{len(data)}] Sukces: {string_id} -> UUID: {point_id}")
            
        except Exception as e:
            print(f"[{i+1}/{len(data)}] BŁĄD przy zadaniu {string_id}: {e}")
